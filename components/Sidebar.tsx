...
export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabChange, className = '', hasActiveProject, onStartNewProject, onLogout }) => {
  const tabs: { id: MainTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'PROJECT', label: hasActiveProject ? '내 프로젝트' : '창업 시작하기', icon: <Rocket size={20} /> },
    { id: 'CONSULTING', label: '상담', icon: <MessageSquare size={20} /> },
    { id: 'MORE', label: '더보기', icon: <Settings size={20} /> },
    { id: 'MYPAGE', label: '마이페이지', icon: <User size={20} /> },
  ];
