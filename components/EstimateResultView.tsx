        ...
        <div className="fixed inset-0 bg-slate-50 z-50 overflow-y-auto animate-in fade-in duration-500">
            <div className="max-w-md mx-auto min-h-screen bg-white shadow-xl flex flex-col">
                ...
                <div className="px-5 -mt-14 relative z-10">
                    ...
                    {/* 상권 분석 상세 */}
                    {hasAI && (
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-4 space-y-4">
                            {/* 제목 */}
                            <p className="text-[13px] font-bold text-slate-800">상권 분석</p>

                            {/* 등급 근거 */}
                            <p className="text-sm text-slate-500 leading-relaxed">{data.aiReport!.locationAnalysis.gradeReason}</p>

                            {/* 타겟 / 피크 */}
                            <div className="space-y-2.5">
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-slate-400 w-14 shrink-0">타겟</span>
                                    <span className="text-[13px] text-slate-700">{data.aiReport!.locationAnalysis.targetCustomer}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-slate-400 w-14 shrink-0">피크</span>
                                    <span className="text-[13px] text-slate-700">{data.aiReport!.locationAnalysis.peakHours}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-1.5">
                                {data.aiReport!.locationAnalysis.strengths.map((s, i) => (
                                    <p key={`s${i}`} className="text-[13px] text-slate-600 pl-3 border-l-2 border-blue-400">{s}</p>
                                ))}
                                {data.aiReport!.locationAnalysis.weaknesses.map((w, i) => (
                                    <p key={`w${i}`} className="text-[13px] text-slate-400 pl-3 border-l-2 border-slate-200">{w}</p>
                                ))}
                            </div>
