...
import {
  ...
  ClipboardList, Users, Banknote, Download
} from 'lucide-react';
      ...
      return (
        <div className="min-h-screen bg-slate-50">
          ...
          <div className="p-4 space-y-4">
            ...
            {/* PDF 견적서 다운로드 */}
            <div className="bg-white rounded-xl border border-slate-100 p-5">
              <h4 className="font-bold mb-3">견적서 PDF</h4>
              <PDFDownloadLink
                document={
                  <EstimatePDFDocument
                    customerName={user?.name || '고객'}
                    totalCostRange={{ min: selectedQuote.totalCost * 0.8, max: selectedQuote.totalCost * 1.2 }}
                    locationData={{ region: '서울시 강남구', analysis: [] }}
                    costBreakdown={[
                      { label: '물품비', min: selectedQuote.itemsCost, max: selectedQuote.itemsCost },
                      { label: '물류비', min: selectedQuote.logisticsCost, max: selectedQuote.logisticsCost },
                      { label: '설치비', min: selectedQuote.installationCost, max: selectedQuote.installationCost },
                    ].filter(i => i.min > 0)}
                    checklist={{ readyCount: 0, worryCount: 0, worryItems: [], readyItems: [] }}
                    projectName={selectedQuote.packageName}
                  />
                }
                fileName={`opening_estimate_${selectedQuote.packageName}_${selectedQuote.date}.pdf`}
                className="w-full"
              >
                <button className="w-full flex items-center justify-center gap-2 bg-brand-600 text-white rounded-xl py-3 font-bold hover:bg-brand-700 transition-colors">
                  <Download size={18} />
                  견적서 PDF 다운로드
                </button>
              </PDFDownloadLink>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="p-5 space-y-6">
        <div className="text-center py-6">
          <img src="/favicon-new.png" alt="오프닝 로고" className="w-20 h-20 mx-auto mb-4 rounded-2xl" />
